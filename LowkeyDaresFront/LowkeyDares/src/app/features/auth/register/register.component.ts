  ...
  template: `
    <div class="auth-shell">
      ...
      <div class="auth-card fade-up">
        <h1>forge your path</h1>
        <p class="subtitle">New challenger? Register & unleash your dares.</p>
        <form (ngSubmit)="onSubmit()" class="auth-form">
          <div class="form-group">
            <label class="form-label">USERNAME</label>
            <input type="text" [(ngModel)]="username" name="username" placeholder="DareViking" required />
          </div>
          <div class="form-group">
            <label class="form-label">EMAIL</label>
            <input type="email" [(ngModel)]="email" name="email" placeholder="rush@lowkeydares.com" required />
          </div>
          <div class="form-group">
            <label class="form-label">PASSWORD</label>
            <input type="password" [(ngModel)]="password" name="password" placeholder="create strong code" required minlength="6" />
          </div>
          <div *ngIf="error()" class="auth-error">{{ error() }}</div>
          <button type="submit" class="btn btn-primary btn-full" [disabled]="loading()">
            <span *ngIf="loading()" class="spinner"></span>
            <span *ngIf="!loading()">🔥 START THE CHALLENGE 🔥</span>
          </button>
        </form>
        <p class="auth-switch">Already a darelord? <a routerLink="/auth/login">Sign in</a></p>
      </div>
    </div>
  `,
  styles: [`
    .auth-shell {
      ...
      background: radial-gradient(circle at 80% 20%, #0D1119, #020304);
      padding: 2rem 1rem;
      position: relative;
    }
    .auth-shell::before {
